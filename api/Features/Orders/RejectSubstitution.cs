...
public class RejectSubstitutionHandler(InMemoryStore store)
{
    public IResult Handle(string orderId, string subId)
    {
        ...
        sub.CustomerApproved = false;
        sub.ApprovedAt = DateTime.UtcNow;
        return Results.Ok(new { substitutionId = subId, customerApproved = false, approvedAt = sub.ApprovedAt });
    }
}
